...
# try:
#     test_gpu = pytest.config.getoption("--gpu")
#     print("HERE", test_gpu)
# except AttributeError:
#     test_gpu = False
#     print("pytest object has no attribute 'config'. Going ahead anyway.")

test_gpu = False

if test_gpu:
    try:
        from .cuda import pyvfit_libgpu as acc_lib
        HAVE_CUDA = True
        print("have cuda")
    except ImportError:
        HAVE_CUDA = False
    ...
    from .cuda import pyvfit_libcpu as acc_lib
    HAVE_CUDA = True
...
def test_rotix():
    ...
    uv = Imager.pixel_coordinates(maxuv, size).astype(ARR_TYPE)
    udat, vdat = create_sampling_points(nsamples, maxuv/4.8)
    ...
    ui, vi = vfit_ffun.get_rotix_nf(uv, uv, udat, vdat, len(udat), size)
...
@pytest.mark.skipif((not HAVE_CUDA) or ("cuda_rotix_interpolate" not in dir(acc_lib)), reason="requires cuda and the cuda_rotix_interpolate function")
...
@pytest.mark.skipif((not HAVE_CUDA) or ("cuda_shift_fft_shift_apply_phase" not in dir(acc_lib)), reason="requires cuda and the cuda_shift_fft_shift_apply_phase function")
