...
Let's say you have an observational dataset of `N` visibility points located at (|u_j|, |v_j|), with `j=1...N` and |u_j|, |v_j|
expressed in units of the observing wavelength. `V`:sub:`obs, j` is the `j-th` complex visibility with associated theoretical weight |w_j|.
If you want to compute the visibilities of a model `image` in the same (|u_j|, |v_j|) locations of the observations,
...
If `I(R)` is the radial brightness profile, the command is as simple as:
